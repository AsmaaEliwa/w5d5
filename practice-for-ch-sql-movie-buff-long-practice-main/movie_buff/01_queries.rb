...
def harrison_ford
  ...
  # as a lead actor.
   Actor.joins(:movies).select("movies.id,title").where("name='Harrison Ford' AND ord!=1")
...
def biggest_cast
  ...
  # actors).
  Movie.joins(:castings).group('movies.id').order('COUNT(castings.id) DESC').limit(3).select("movies.id,title")
